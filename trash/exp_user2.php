...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title>Exp_user - <?php echo SITENAME; ?></title>
 ...
 <span class="hidden-phone"> Columns </span> <b class="caret"></b> </div>
        <ul class="dropdown-menu">
		  ...
		  <label class="checkbox">
				...
				<input type="checkbox" class="columncontrol" checked="checked" value="user_name"><span class="lbl"> User name </span></label>  <label class="checkbox">
				<input type="checkbox" class="columncontrol" checked="checked" value="category_id"><span class="lbl"> Cat id </span></label>
          </li>
        </ul>
      </span>
	  <button data-target="#myModal" data-toggle="modal" id="addnewrec"  class="pull-right btn btn-small btn-primary">Add</button>
							<h1>Exp_user<small><i class="icon-double-angle-right"></i> Exp_user</small></h1>
						</div><!--/page-header-->
						<div class="row-fluid" >
<!-- PAGE CONTENT BEGINS HERE -->
<div style="100%" id="section_exp_user">
<div class="row-fluid" >
	...
	<!--  ACTION BOX ENDS HERE -->
		...
		<div class="control-group">
				...
				<div class="controls ">
					<input type="text"  id="pag_search_exp_user--id" class="pag_filter input-medium search-query " placeholder="Search Id">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > User name</label>
				<div class="controls ">
					<input type="text"  id="pag_search_exp_user--user_name" class="pag_filter input-medium search-query " placeholder="Search User name">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Cat id</label>
				<div class="controls ">
					<input type="text"  id="pag_search_exp_user--category_id" class="pag_filter input-medium search-query " placeholder="Search Cat id">
				</div>
		</div>
								<div style="clear:both;"></div>
                                <div class="span6">
				...
				<table id="table_report" class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							...
							<th data-bind="click:sortby.bind($data, 'exp_user.id')" style="cursor: pointer; display: none;" class="col_id"><i class="sort_icon" id="sorticon_id"></i>Id</th> 
							<th data-bind="click:sortby.bind($data, 'exp_user.category_id')" style="cursor: pointer; display: none;" class="col_category_id"><i class="sort_icon" id="sorticon_category_id"></i>Cat id</th> 
							<th>Action</th>				
						</tr>
					</thead>
					<tbody data-bind="foreach:exp_user">
						<tr>
...
<div class="control-group">
            <label class="control-label" > User name</label>
            <div class="controls">
              <input  class="span4"  value="" type="text"  id="pag_search_user_name" name="user_name" placeholder="User name" required>
			  </div>
          </div>
        </form>
      </div>
...
// CHECKLIST MODEL 
	function AppViewModelexp_user(exp_user) {
	var self = this;
	...
	// ADD NEW ITEM
   self.addexp_user = function() {
		var fname="";
        self.exp_user.push({ id: ko.observable(),checklist_name: ko.observable() });
		self.saveexp_user();
    };
	...
	self.deleteselecteddata = function(){
			$.ajax({
			type: "POST",
			url: "services/exp_user_delete.php",
	...
	// Looping up all the filters start here. 
		...
		$.ajax({
					type: "POST",
					url: "services/exp_user_fetch.php",
					data : pag_filters,
					dataType: 'json'
					}).done(
					function( data ) 
					{
						...
						ko.mapping.fromJS(viewModel, self.exp_user);
	...
	// SAVE THE MODEL
	self.saveexp_user = function() {
		$('#message').show();
		//alert(ko.toJSON(self.exp_user));
		mydataa = ko.toJSON(self.exp_user);
		$.ajax({
		  type: "POST",
		  url: "services/exp_user_update.php",
		  data: { mydata: mydataa }
			}).done(function( msg ) {
...
<script language="javascript">
$(document).ready(function(){
	var exp_user = new AppViewModelexp_user(ko.mapping.fromJS([]));
	ko.applyBindings((exp_user),$("#section_exp_user")[0]);
	exp_user.loaddata();
	exp_user.update_pagination();
});
</script>
<script type="text/javascript">
$(document).ready(function() {
