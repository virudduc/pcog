...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title>Attribute_management - <?php echo SITENAME; ?></title>
 ...
 <span class="hidden-phone"> Columns </span> <b class="caret"></b> </div>
        <ul class="dropdown-menu">
		  ...
		  <label class="checkbox">
				...
				<input type="checkbox" class="columncontrol" checked="checked" value="category_id"><span class="lbl"> Category Name </span></label>  <label class="checkbox">
				<input type="checkbox" class="columncontrol" checked="checked" value="field_name"><span class="lbl"> Field Name </span></label>  <label class="checkbox">
				<input type="checkbox" class="columncontrol" checked="checked" value="attribute_type"><span class="lbl"> Attribute Type </span></label>  <label class="checkbox">
				<input type="checkbox" class="columncontrol" checked="checked" value="is_integer"><span class="lbl"> Is Integer </span></label>  <label class="checkbox">
				<input type="checkbox" class="columncontrol" checked="checked" value="is_required"><span class="lbl"> Is Required </span></label>
          </li>
        </ul>
      </span>
	  <button data-target="#myModal" data-toggle="modal" id="addnewrec"  class="pull-right btn btn-small btn-primary">Add</button>
							<h1>Attribute Management<small><i class="icon-double-angle-right"></i> Product Management</small></h1>
						</div><!--/page-header-->
						<div class="row-fluid" >
<!-- PAGE CONTENT BEGINS HERE -->
<div style="100%" id="section_attribute_management">
<div class="row-fluid" >
	...
	<!--  ACTION BOX ENDS HERE -->
		...
		<div class="control-group">
				...
				<div class="controls ">
					<input type="text"  id="pag_search_attribute_management--id" class="pag_filter input-medium search-query " placeholder="Search Id">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Category Name</label>
				<div class="controls ">
					<input type="text"  id="pag_search_attribute_management--category_id" class="pag_filter input-medium search-query " placeholder="Search Category Name">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Field Name</label>
				<div class="controls ">
					<input type="text"  id="pag_search_attribute_management--field_name" class="pag_filter input-medium search-query " placeholder="Search Field Name">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Attribute Type</label>
				<div class="controls ">
					<input type="text"  id="pag_search_attribute_management--attribute_type" class="pag_filter input-medium search-query " placeholder="Search Attribute Type">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Is Integer</label>
				<div class="controls ">
					<input type="text"  id="pag_search_attribute_management--is_integer" class="pag_filter input-medium search-query " placeholder="Search Is Integer">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Is Required</label>
				<div class="controls ">
					<input type="text"  id="pag_search_attribute_management--is_required" class="pag_filter input-medium search-query " placeholder="Search Is Required">
				</div>
		</div>
								<div style="clear:both;"></div>
                                <div class="span6">
				...
				<table id="table_report" class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							...
							<th data-bind="click:sortby.bind($data, 'attribute_management.category_id')" style="cursor: pointer; display: none;" class="col_category_id"><i class="sort_icon" id="sorticon_category_id"></i>Category Name</th> 
							<th data-bind="click:sortby.bind($data, 'attribute_management.field_name')" style="cursor: pointer; display: none;" class="col_field_name"><i class="sort_icon" id="sorticon_field_name"></i>Field Name</th> 
							<th data-bind="click:sortby.bind($data, 'attribute_management.attribute_type')" style="cursor: pointer; display: none;" class="col_attribute_type"><i class="sort_icon" id="sorticon_attribute_type"></i>Attribute Type</th> 
							<th>Action</th>				
						</tr>
					</thead>
					<tbody data-bind="foreach:attribute_management">
						<tr>
							<td class="center sorting_1">
								<label><input type="checkbox" class="title idcheck" name="id[]"  data-bind="attr:{value: id}" id="id[]" value="" /><span class="lbl"></span></label>
							</td>
										<td data-bind="text: category_id" class="col_category_id" style="display: none;"></td> 
										<td data-bind="text: field_name" class="col_field_name" style="display: none;"></td> 
										<td data-bind="text: attribute_type" class="col_attribute_type" style="display: none;"></td> 
							<td>
...
<div class="control-group">
            <label class="control-label" >Category Name</label>
            <div class="controls">
          <select  id="pag_search_category_id" name="category_id" placeholder="Category Name" >
			<?php
				$cms_product_categories_query ="select * from cms_product_categories";
				$cms_product_categories_rs = mysql_query($cms_product_categories_query) or die(mysql_error()."</br> Failed Query".$project_query );
				//Loop Placed here. 
				while($cms_product_categories_row = mysql_fetch_array($cms_product_categories_rs))
				{
				?> 
				<option value="<?php echo $cms_product_categories_row['id'] ;?>"><?php echo $cms_product_categories_row['category_name'] ;?></option>
				<?php 
				}
			?>
         </select>
...
<div class="control-group">
            <label class="control-label" > Field Name</label>
            <div class="controls">
              <input  class="span4"  value="" type="text"  id="pag_search_field_name" name="field_name" placeholder="Field Name" required>
			  </div>
          </div>
<div class="control-group">
            <label class="control-label" >Attribute Type</label>
            <div class="controls">
          <select  id="pag_search_attribute_type" name="attribute_type" placeholder="Attribute Type" >
			<?php
				$attribute_type_query ="select * from attribute_type";
				$attribute_type_rs = mysql_query($attribute_type_query) or die(mysql_error()."</br> Failed Query".$project_query );
				//Loop Placed here. 
				while($attribute_type_row = mysql_fetch_array($attribute_type_rs))
				{
				?> 
				<option value="<?php echo $attribute_type_row['id'] ;?>"><?php echo $attribute_type_row['attribute_type'] ;?></option>
				<?php 
				}
			?>
         </select>
...
<div class="control-group">
            <label class="control-label" > Is Integer</label>
            <div class="controls">
             <input type="checkbox"  id="pag_search_is_integer"   name="is_integer" placeholder="Is Integer" >
			 </div>
          </div>
<div class="control-group">
            <label class="control-label" > Is Required</label>
            <div class="controls">
             <input type="checkbox"  id="pag_search_is_required"   name="is_required" placeholder="Is Required" >
			 </div>
          </div>
        </form>
      </div>
...
// CHECKLIST MODEL 
	function AppViewModelattribute_management(attribute_management) {
	var self = this;
		self.attribute_management = attribute_management;
	...
	// ADD NEW ITEM
   self.addattribute_management = function() {
		var fname="";
        self.attribute_management.push({ id: ko.observable(),checklist_name: ko.observable() });
		self.saveattribute_management();
    };
	...
	self.deleteselecteddata = function(){
			$.ajax({
			type: "POST",
			url: "services/attribute_management_delete.php",
	...
	self.savenew = function(){
		$.ajax({
		  type: "POST",
		   url: "services/attribute_management_new.php",
	...
	// Looping up all the filters start here. 
		...
		$.ajax({
					type: "POST",
					url: "services/attribute_management_fetch.php",
					data : pag_filters,
					dataType: 'json'
					}).done(
					function( data ) 
					{
						...
						ko.mapping.fromJS(viewModel, self.attribute_management);
	...
	// SAVE THE MODEL
	self.saveattribute_management = function() {
		$('#message').show();
		//alert(ko.toJSON(self.attribute_management));
		mydataa = ko.toJSON(self.attribute_management);
		$.ajax({
		  type: "POST",
		  url: "services/attribute_management_update.php",
		  data: { mydata: mydataa }
			}).done(function( msg ) {
	...
	self.removeItem = function() {
        self.attribute_management.remove(this);
...
<script language="javascript">
$(document).ready(function(){
	var attribute_management = new AppViewModelattribute_management(ko.mapping.fromJS([]));
	ko.applyBindings((attribute_management),$("#section_attribute_management")[0]);
	attribute_management.loaddata();
	attribute_management.update_pagination();
});
</script>
<script type="text/javascript">
$(document).ready(function() {
