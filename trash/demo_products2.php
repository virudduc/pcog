...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title>Demo_products - <?php echo SITENAME; ?></title>
			...
			<div id="main-content" class="clearfix">
					<div id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li class="active">Demo_products</li>
						</ul><!--.breadcrumb-->
						<div id="nav-search">
 ...
 <span class="hidden-phone"> Columns </span> <b class="caret"></b> </div>
	  ...
	  <button data-target="#myModal" data-toggle="modal" id="addnewrec"  class="pull-right btn btn-small btn-primary">Add</button>
							<h1>Demo_products<small><i class="icon-double-angle-right"></i> Demo_products</small></h1>
						</div><!--/page-header-->
						<div class="row-fluid" >
<!-- PAGE CONTENT BEGINS HERE -->
<div style="100%" id="section_demo_products">
<div class="row-fluid" >
		...
		<div class="control-group">
			...
			<form name="maintableform" id="maintableform">
				<table id="table_report" class="table table-striped table-bordered table-hover">
					...
					<tbody data-bind="foreach:demo_products">
						<tr>
...
// CHECKLIST MODEL 
	function AppViewModeldemo_products(demo_products) {
	var self = this;
	...
	// ADD NEW ITEM
   self.adddemo_products = function() {
		var fname="";
        self.demo_products.push({ id: ko.observable(),checklist_name: ko.observable() });
		self.savedemo_products();
    };
	...
	self.deleteselecteddata = function(){
			$.ajax({
			type: "POST",
			url: "services/demo_products_delete.php",
	...
	self.savenew = function(){
		$.ajax({
		  type: "POST",
		   url: "services/demo_products_new.php",
	...
	// Looping up all the filters start here. 
		...
		$.ajax({
					type: "POST",
					url: "services/demo_products_fetch.php",
					data : pag_filters,
					dataType: 'json'
					}).done(
					function( data ) 
					{
						...
						ko.mapping.fromJS(viewModel, self.demo_products);
	...
	// SAVE THE MODEL
	self.savedemo_products = function() {
		$('#message').show();
		//alert(ko.toJSON(self.demo_products));
		mydataa = ko.toJSON(self.demo_products);
		$.ajax({
		  type: "POST",
		  url: "services/demo_products_update.php",
		  data: { mydata: mydataa }
			}).done(function( msg ) {
	...
	self.removeItem = function() {
        self.demo_products.remove(this);
...
<script language="javascript">
$(document).ready(function(){
	var demo_products = new AppViewModeldemo_products(ko.mapping.fromJS([]));
	ko.applyBindings((demo_products),$("#section_demo_products")[0]);
	demo_products.loaddata();
	demo_products.update_pagination();
});
</script>
<script type="text/javascript">
$(document).ready(function() {
