...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title>Member_message - <?php echo SITENAME; ?></title>
 ...
 <span class="hidden-phone"> Columns </span> <b class="caret"></b> </div>
        <ul class="dropdown-menu">
		  ...
		  <label class="checkbox">
				...
				<input type="checkbox" class="columncontrol" checked="checked" value="message_to"><span class="lbl"> To </span></label>  <label class="checkbox">
				<input type="checkbox" class="columncontrol" checked="checked" value="message_from"><span class="lbl"> From </span></label>  <label class="checkbox">
				<input type="checkbox" class="columncontrol" checked="checked" value="message_subject"><span class="lbl"> Subject </span></label>  <label class="checkbox">
				<input type="checkbox" class="columncontrol" checked="checked" value="message_text"><span class="lbl"> Message </span></label>
          </li>
        </ul>
      </span>
	  <button data-target="#myModal" data-toggle="modal" id="addnewrec"  class="pull-right btn btn-small btn-primary">Add</button>
							<h1>Message<small><i class="icon-double-angle-right"></i> Management</small></h1>
						</div><!--/page-header-->
						<div class="row-fluid" >
<!-- PAGE CONTENT BEGINS HERE -->
<div style="100%" id="section_member_message">
<div class="row-fluid" >
	...
	<!--  ACTION BOX ENDS HERE -->
		...
		<div class="control-group">
				...
				<div class="controls ">
					<input type="text"  id="pag_search_member_message--id" class="pag_filter input-medium search-query " placeholder="Search Id">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > To</label>
				<div class="controls ">
					<input type="text"  id="pag_search_member_message--message_to" class="pag_filter input-medium search-query " placeholder="Search To">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > From</label>
				<div class="controls ">
					<input type="text"  id="pag_search_member_message--message_from" class="pag_filter input-medium search-query " placeholder="Search From">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Subject</label>
				<div class="controls ">
					<input type="text"  id="pag_search_member_message--message_subject" class="pag_filter input-medium search-query " placeholder="Search Subject">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Message</label>
				<div class="controls ">
					<input type="text"  id="pag_search_member_message--message_text" class="pag_filter input-medium search-query " placeholder="Search Message">
				</div>
		</div>
								<div style="clear:both;"></div>
                                <div class="span6">
				...
				<table id="table_report" class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							...
							<th data-bind="click:sortby.bind($data, 'member_message.message_to')" style="cursor: pointer; display: none;" class="col_message_to"><i class="sort_icon" id="sorticon_message_to"></i>To</th> 
							<th data-bind="click:sortby.bind($data, 'member_message.message_from')" style="cursor: pointer; display: none;" class="col_message_from"><i class="sort_icon" id="sorticon_message_from"></i>From</th> 
							<th data-bind="click:sortby.bind($data, 'member_message.message_subject')" style="cursor: pointer; display: none;" class="col_message_subject"><i class="sort_icon" id="sorticon_message_subject"></i>Subject</th> 
							<th>Action</th>				
						</tr>
					</thead>
					<tbody data-bind="foreach:member_message">
						<tr>
							<td class="center sorting_1">
								<label><input type="checkbox" class="title idcheck" name="id[]"  data-bind="attr:{value: id}" id="id[]" value="" /><span class="lbl"></span></label>
							</td>
										<td data-bind="text: message_to" class="col_message_to" style="display: none;"></td> 
										<td data-bind="text: message_from" class="col_message_from" style="display: none;"></td> 
										<td data-bind="text: message_subject" class="col_message_subject" style="display: none;"></td> 
							<td>
...
<div class="control-group">
            <label class="control-label" >To</label>
            <div class="controls">
          <select  id="pag_search_message_to" name="message_to" placeholder="To" >
			<?php
...
<div class="control-group">
            <label class="control-label" >From</label>
            <div class="controls">
          <select  id="pag_search_message_from" name="message_from" placeholder="From" >
			<?php
				$vt_users_query ="select * from vt_users";
				$vt_users_rs = mysql_query($vt_users_query) or die(mysql_error()."</br> Failed Query".$project_query );
				//Loop Placed here. 
...
<div class="control-group">
            <label class="control-label" > Subject</label>
            <div class="controls">
              <input  class="span4"  value="" type="text"  id="pag_search_message_subject" name="message_subject" placeholder="Subject" required>
			  </div>
          </div>
<div class="control-group">
            <label class="control-label" > Message</label>
            <div class="controls">
       <textarea class="span4" id="pag_search_message_text" name="message_text" placeholder="Message" ></textarea>
	   </div>
          </div>
        </form>
      </div>
...
// CHECKLIST MODEL 
	function AppViewModelmember_message(member_message) {
	var self = this;
	...
	// ADD NEW ITEM
   self.addmember_message = function() {
		var fname="";
        self.member_message.push({ id: ko.observable(),checklist_name: ko.observable() });
		self.savemember_message();
    };
	...
	self.deleteselecteddata = function(){
			$.ajax({
			type: "POST",
			url: "services/member_message_delete.php",
	...
	self.savenew = function(){
		$.ajax({
		  type: "POST",
		   url: "services/member_message_new.php",
	...
	// Looping up all the filters start here. 
		...
		$.ajax({
					type: "POST",
					url: "services/member_message_fetch.php",
					data : pag_filters,
					dataType: 'json'
					}).done(
					function( data ) 
					{
						...
						ko.mapping.fromJS(viewModel, self.member_message);
	...
	// SAVE THE MODEL
	self.savemember_message = function() {
		$('#message').show();
		//alert(ko.toJSON(self.member_message));
		mydataa = ko.toJSON(self.member_message);
		$.ajax({
		  type: "POST",
		  url: "services/member_message_update.php",
		  data: { mydata: mydataa }
			}).done(function( msg ) {
	...
	self.removeItem = function() {
        self.member_message.remove(this);
...
<script language="javascript">
$(document).ready(function(){
	var member_message = new AppViewModelmember_message(ko.mapping.fromJS([]));
	ko.applyBindings((member_message),$("#section_member_message")[0]);
	member_message.loaddata();
	member_message.update_pagination();
});
</script>
<script type="text/javascript">
$(document).ready(function() {
