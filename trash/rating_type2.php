...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title>Rating_type - <?php echo SITENAME; ?></title>
			...
			<div id="main-content" class="clearfix">
					<div id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li class="active">Rating Type</li>
						</ul><!--.breadcrumb-->
						<div id="nav-search">
 ...
 <span class="hidden-phone"> Columns </span> <b class="caret"></b> </div>
        <ul class="dropdown-menu">
		  ...
		  <label class="checkbox">
				...
				<input type="checkbox" class="columncontrol" checked="checked" value="rating_type"><span class="lbl"> Rating type </span></label>
          </li>
        </ul>
      </span>
	  <button data-target="#myModal" data-toggle="modal" id="addnewrec"  class="pull-right btn btn-small btn-primary">Add</button>
							<h1>Rating Type<small><i class="icon-double-angle-right"></i> Rating Type</small></h1>
						</div><!--/page-header-->
						<div class="row-fluid" >
<!-- PAGE CONTENT BEGINS HERE -->
<div style="100%" id="section_rating_type">
<div class="row-fluid" >
	...
	<!--  ACTION BOX ENDS HERE -->
		...
		<div class="control-group">
				...
				<div class="controls ">
					<input type="text"  id="pag_search_rating_type--id" class="pag_filter input-medium search-query " placeholder="Search Id">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Rating type</label>
				<div class="controls ">
					<input type="text"  id="pag_search_rating_type--rating_type" class="pag_filter input-medium search-query " placeholder="Search Rating type">
				</div>
		</div>
								<div style="clear:both;"></div>
                                <div class="span6">
				...
				<table id="table_report" class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							...
							<th data-bind="click:sortby.bind($data, 'rating_type.id')" style="cursor: pointer; display: none;" class="col_id"><i class="sort_icon" id="sorticon_id"></i>Id</th> 
							<th data-bind="click:sortby.bind($data, 'rating_type.rating_type')" style="cursor: pointer; display: none;" class="col_rating_type"><i class="sort_icon" id="sorticon_rating_type"></i>Rating type</th> 
							<th>Action</th>				
						</tr>
					</thead>
					<tbody data-bind="foreach:rating_type">
						<tr>
							<td class="center sorting_1">
								<label><input type="checkbox" class="title idcheck" name="id[]"  data-bind="attr:{value: id}" id="id[]" value="" /><span class="lbl"></span></label>
										...
										<td data-bind="text: rating_type" class="col_rating_type" style="display: none;"></td> 
							<td>
...
<div class="control-group">
            <label class="control-label" > Rating type</label>
            <div class="controls">
              <input  class="span4"  value="" type="text"  id="pag_search_rating_type" name="rating_type" placeholder="Rating type" required>
			  </div>
          </div>
        </form>
      </div>
...
// CHECKLIST MODEL 
	function AppViewModelrating_type(rating_type) {
	var self = this;
	...
	// ADD NEW ITEM
   self.addrating_type = function() {
		var fname="";
        self.rating_type.push({ id: ko.observable(),checklist_name: ko.observable() });
		self.saverating_type();
    };
	...
	self.deleteselecteddata = function(){
			$.ajax({
			type: "POST",
			url: "services/rating_type_delete.php",
	...
	self.savenew = function(){
		$.ajax({
		  type: "POST",
		   url: "services/rating_type_new.php",
	...
	// Looping up all the filters start here. 
		...
		$.ajax({
					type: "POST",
					url: "services/rating_type_fetch.php",
					data : pag_filters,
					dataType: 'json'
					}).done(
					function( data ) 
					{
						...
						ko.mapping.fromJS(viewModel, self.rating_type);
	...
	// SAVE THE MODEL
	self.saverating_type = function() {
		$('#message').show();
		//alert(ko.toJSON(self.rating_type));
		mydataa = ko.toJSON(self.rating_type);
		$.ajax({
		  type: "POST",
		  url: "services/rating_type_update.php",
		  data: { mydata: mydataa }
			}).done(function( msg ) {
...
<script language="javascript">
$(document).ready(function(){
	var rating_type = new AppViewModelrating_type(ko.mapping.fromJS([]));
	ko.applyBindings((rating_type),$("#section_rating_type")[0]);
	rating_type.loaddata();
	rating_type.update_pagination();
});
</script>
<script type="text/javascript">
$(document).ready(function() {
