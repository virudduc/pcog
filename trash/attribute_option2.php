...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title>Attribute_option - <?php echo SITENAME; ?></title>
			...
			<div id="main-content" class="clearfix">
					<div id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li class="active">Attribute Option</li>
						</ul><!--.breadcrumb-->
						<div id="nav-search">
 ...
 <span class="hidden-phone"> Columns </span> <b class="caret"></b> </div>
        <ul class="dropdown-menu">
		  ...
		  <label class="checkbox">
				...
				<input type="checkbox" class="columncontrol" checked="checked" value="attribute_id"><span class="lbl"> Attribute Id </span></label>  <label class="checkbox">
				<input type="checkbox" class="columncontrol" checked="checked" value="option_value"><span class="lbl"> Option value </span></label>  <label class="checkbox">
				<input type="checkbox" class="columncontrol" checked="checked" value="option_label"><span class="lbl"> Option label </span></label>
          </li>
        </ul>
      </span>
	  <button data-target="#myModal" data-toggle="modal" id="addnewrec"  class="pull-right btn btn-small btn-primary">Add</button>
							<h1>Attribute Option<small><i class="icon-double-angle-right"></i> Attribute Management</small></h1>
						</div><!--/page-header-->
						<div class="row-fluid" >
<!-- PAGE CONTENT BEGINS HERE -->
<div style="100%" id="section_attribute_option">
<div class="row-fluid" >
	...
	<!--  ACTION BOX ENDS HERE -->
		...
		<div class="control-group">
				...
				<div class="controls ">
					<input type="text"  id="pag_search_attribute_option--id" class="pag_filter input-medium search-query " placeholder="Search Id">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Attribute Id</label>
				<div class="controls ">
					<input type="text"  id="pag_search_attribute_option--attribute_id" class="pag_filter input-medium search-query " placeholder="Search Attribute Id">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Option value</label>
				<div class="controls ">
					<input type="text"  id="pag_search_attribute_option--option_value" class="pag_filter input-medium search-query " placeholder="Search Option value">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Option label</label>
				<div class="controls ">
					<input type="text"  id="pag_search_attribute_option--option_label" class="pag_filter input-medium search-query " placeholder="Search Option label">
				</div>
		</div>
								<div style="clear:both;"></div>
                                <div class="span6">
				...
				<table id="table_report" class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							...
							<th data-bind="click:sortby.bind($data, 'attribute_option.attribute_id')" style="cursor: pointer; display: none;" class="col_attribute_id"><i class="sort_icon" id="sorticon_attribute_id"></i>Attribute Id</th> 
							<th data-bind="click:sortby.bind($data, 'attribute_option.option_value')" style="cursor: pointer; display: none;" class="col_option_value"><i class="sort_icon" id="sorticon_option_value"></i>Option value</th> 
							<th data-bind="click:sortby.bind($data, 'attribute_option.option_label')" style="cursor: pointer; display: none;" class="col_option_label"><i class="sort_icon" id="sorticon_option_label"></i>Option label</th> 
							<th>Action</th>				
						</tr>
					</thead>
					<tbody data-bind="foreach:attribute_option">
						<tr>
							<td class="center sorting_1">
								<label><input type="checkbox" class="title idcheck" name="id[]"  data-bind="attr:{value: id}" id="id[]" value="" /><span class="lbl"></span></label>
							</td>
										<td data-bind="text: attribute_id" class="col_attribute_id" style="display: none;"></td> 
										<td data-bind="text: option_value" class="col_option_value" style="display: none;"></td> 
										<td data-bind="text: option_label" class="col_option_label" style="display: none;"></td> 
							<td>
...
<div class="control-group">
            <label class="control-label" >Attribute Id</label>
            <div class="controls">
          <select  id="pag_search_attribute_id" name="attribute_id" placeholder="Attribute Id" >
			<?php
				$attribute_management_query ="select * from attribute_management";
				$attribute_management_rs = mysql_query($attribute_management_query) or die(mysql_error()."</br> Failed Query".$project_query );
				//Loop Placed here. 
				while($attribute_management_row = mysql_fetch_array($attribute_management_rs))
				{
				?> 
				<option value="<?php echo $attribute_management_row['id'] ;?>"><?php echo $attribute_management_row['id'] ;?></option>
				<?php 
				}
			?>
         </select>
...
<div class="control-group">
            <label class="control-label" > Option value</label>
            <div class="controls">
              <input  class="span4"  value="" type="text"  id="pag_search_option_value" name="option_value" placeholder="Option value" required>
			  </div>
          </div>
<div class="control-group">
            <label class="control-label" > Option label</label>
            <div class="controls">
              <input  class="span4"  value="" type="text"  id="pag_search_option_label" name="option_label" placeholder="Option label" required>
			  </div>
          </div>
        </form>
      </div>
...
// CHECKLIST MODEL 
	function AppViewModelattribute_option(attribute_option) {
	var self = this;
		self.attribute_option = attribute_option;
	...
	// ADD NEW ITEM
   self.addattribute_option = function() {
		var fname="";
        self.attribute_option.push({ id: ko.observable(),checklist_name: ko.observable() });
		self.saveattribute_option();
    };
	...
	self.deleteselecteddata = function(){
			$.ajax({
			type: "POST",
			url: "services/attribute_option_delete.php",
	...
	self.savenew = function(){
		$.ajax({
		  type: "POST",
		   url: "services/attribute_option_new.php",
	...
	// Looping up all the filters start here. 
		...
		$.ajax({
					type: "POST",
					url: "services/attribute_option_fetch.php",
					data : pag_filters,
					dataType: 'json'
					}).done(
					function( data ) 
					{
						...
						ko.mapping.fromJS(viewModel, self.attribute_option);
	...
	// SAVE THE MODEL
	self.saveattribute_option = function() {
		$('#message').show();
		//alert(ko.toJSON(self.attribute_option));
		mydataa = ko.toJSON(self.attribute_option);
		$.ajax({
		  type: "POST",
		  url: "services/attribute_option_update.php",
		  data: { mydata: mydataa }
			}).done(function( msg ) {
	...
	self.removeItem = function() {
        self.attribute_option.remove(this);
...
<script language="javascript">
$(document).ready(function(){
	var attribute_option = new AppViewModelattribute_option(ko.mapping.fromJS([]));
	ko.applyBindings((attribute_option),$("#section_attribute_option")[0]);
	attribute_option.loaddata();
	attribute_option.update_pagination();
});
</script>
<script type="text/javascript">
$(document).ready(function() {
