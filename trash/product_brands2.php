...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title>Product_brands - <?php echo SITENAME; ?></title>
 ...
 <span class="hidden-phone"> Columns </span> <b class="caret"></b> </div>
        <ul class="dropdown-menu">
		  ...
		  <label class="checkbox">
				...
				<input type="checkbox" class="columncontrol" checked="checked" value="brand_name"><span class="lbl"> Brand name </span></label>  <label class="checkbox">
				<input type="checkbox" class="columncontrol" checked="checked" value="brand_logo"><span class="lbl"> Brand logo </span></label>  <label class="checkbox">
				<input type="checkbox" class="columncontrol" checked="checked" value="brand_short_desc"><span class="lbl"> Brand short desc </span></label>
          </li>
        </ul>
      </span>
	  <button data-target="#myModal" data-toggle="modal" id="addnewrec"  class="pull-right btn btn-small btn-primary">Add</button>
							<h1>Product Brands<small><i class="icon-double-angle-right"></i> Product Brands</small></h1>
						</div><!--/page-header-->
						<div class="row-fluid" >
<!-- PAGE CONTENT BEGINS HERE -->
<div style="100%" id="section_product_brands">
<div class="row-fluid" >
	...
	<!--  ACTION BOX ENDS HERE -->
		...
		<div class="control-group">
				...
				<div class="controls ">
					<input type="text"  id="pag_search_product_brands--id" class="pag_filter input-medium search-query " placeholder="Search Id">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Brand name</label>
				<div class="controls ">
					<input type="text"  id="pag_search_product_brands--brand_name" class="pag_filter input-medium search-query " placeholder="Search Brand name">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Brand logo</label>
				<div class="controls ">
					<input type="text"  id="pag_search_product_brands--brand_logo" class="pag_filter input-medium search-query " placeholder="Search Brand logo">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Brand short desc</label>
				<div class="controls ">
					<input type="text"  id="pag_search_product_brands--brand_short_desc" class="pag_filter input-medium search-query " placeholder="Search Brand short desc">
				</div>
		</div>
								<div style="clear:both;"></div>
                                <div class="span6">
				...
				<table id="table_report" class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							...
							<th data-bind="click:sortby.bind($data, 'product_brands.id')" style="cursor: pointer; display: none;" class="col_id"><i class="sort_icon" id="sorticon_id"></i>Id</th> 
							<th data-bind="click:sortby.bind($data, 'product_brands.brand_name')" style="cursor: pointer; display: none;" class="col_brand_name"><i class="sort_icon" id="sorticon_brand_name"></i>Brand name</th> 
							<th data-bind="click:sortby.bind($data, 'product_brands.brand_logo')" style="cursor: pointer; display: none;" class="col_brand_logo"><i class="sort_icon" id="sorticon_brand_logo"></i>Brand logo</th> 
							<th data-bind="click:sortby.bind($data, 'product_brands.brand_short_desc')" style="cursor: pointer; display: none;" class="col_brand_short_desc"><i class="sort_icon" id="sorticon_brand_short_desc"></i>Brand short desc</th> 
							<th>Action</th>				
						</tr>
					</thead>
					<tbody data-bind="foreach:product_brands">
						<tr>
							<td class="center sorting_1">
								<label><input type="checkbox" class="title idcheck" name="id[]"  data-bind="attr:{value: id}" id="id[]" value="" /><span class="lbl"></span></label>
										...
										<td data-bind="text: brand_name" class="col_brand_name" style="display: none;"></td> 
										<td data-bind="text: brand_short_desc" class="col_brand_short_desc" style="display: none;"></td> 
							<td>
...
<div class="control-group">
            <label class="control-label" > Brand name</label>
            <div class="controls">
              <input  class="span4"  value="" type="text"  id="pag_search_brand_name" name="brand_name" placeholder="Brand name" required>
			  </div>
          </div>
<div class="control-group">
            <label class="control-label" > Brand short desc</label>
            <div class="controls">
       <textarea class="span4" id="pag_search_brand_short_desc" name="brand_short_desc" placeholder="Brand short desc" ></textarea>
	   </div>
          </div>
        </form>
      </div>
...
// CHECKLIST MODEL 
	function AppViewModelproduct_brands(product_brands) {
	var self = this;
		self.product_brands = product_brands;
	...
	// ADD NEW ITEM
   self.addproduct_brands = function() {
		var fname="";
        self.product_brands.push({ id: ko.observable(),checklist_name: ko.observable() });
		self.saveproduct_brands();
    };
	...
	self.deleteselecteddata = function(){
			$.ajax({
			type: "POST",
			url: "services/product_brands_delete.php",
	...
	self.savenew = function(){
		$.ajax({
		  type: "POST",
		   url: "services/product_brands_new.php",
	...
	// Looping up all the filters start here. 
		...
		$.ajax({
					type: "POST",
					url: "services/product_brands_fetch.php",
					data : pag_filters,
					dataType: 'json'
					}).done(
					function( data ) 
					{
						...
						ko.mapping.fromJS(viewModel, self.product_brands);
	...
	// SAVE THE MODEL
	self.saveproduct_brands = function() {
		$('#message').show();
		//alert(ko.toJSON(self.product_brands));
		mydataa = ko.toJSON(self.product_brands);
		$.ajax({
		  type: "POST",
		  url: "services/product_brands_update.php",
		  data: { mydata: mydataa }
			}).done(function( msg ) {
	...
	self.removeItem = function() {
        self.product_brands.remove(this);
...
<script language="javascript">
$(document).ready(function(){
	var product_brands = new AppViewModelproduct_brands(ko.mapping.fromJS([]));
	ko.applyBindings((product_brands),$("#section_product_brands")[0]);
	product_brands.loaddata();
	product_brands.update_pagination();
});
</script>
<script type="text/javascript">
$(document).ready(function() {
