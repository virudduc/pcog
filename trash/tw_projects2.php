...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title>Tw_projects - <?php echo SITENAME; ?></title>
 ...
 <span class="hidden-phone"> Columns </span> <b class="caret"></b> </div>
        <ul class="dropdown-menu">
		  ...
		  <label class="checkbox">
				...
				<input type="checkbox" class="columncontrol" checked="checked" value="project_name"><span class="lbl"> Project Name </span></label>  <label class="checkbox">
				...
				<input type="checkbox" class="columncontrol" checked="checked" value="modify_date"><span class="lbl"> Modify date </span></label>
          </li>
        </ul>
      </span>
	  <button data-target="#myModal" data-toggle="modal" id="addnewrec"  class="pull-right btn btn-small btn-primary">Add</button>
							<h1>Projects<small><i class="icon-double-angle-right"></i> Managment</small></h1>
						</div><!--/page-header-->
						<div class="row-fluid" >
<!-- PAGE CONTENT BEGINS HERE -->
<div style="100%" id="section_tw_projects">
<div class="row-fluid" >
	...
	<!--  ACTION BOX ENDS HERE -->
		...
		<div class="control-group">
				...
				<div class="controls ">
					<input type="text"  id="pag_search_tw_projects--id" class="pag_filter input-medium search-query " placeholder="Search Id">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Project Name</label>
				<div class="controls ">
					<input type="text"  id="pag_search_tw_projects--project_name" class="pag_filter input-medium search-query " placeholder="Search Project Name">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Create date</label>
				<div class="controls ">
					<input type="text"  id="pag_search_tw_projects--create_date" class="pag_filter input-medium search-query " placeholder="Search Create date">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Modify date</label>
				<div class="controls ">
					<input type="text"  id="pag_search_tw_projects--modify_date" class="pag_filter input-medium search-query " placeholder="Search Modify date">
				</div>
		</div>
								<div style="clear:both;"></div>
                                <div class="span6">
				...
				<table id="table_report" class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							...
							<th data-bind="click:sortby.bind($data, 'tw_projects.id')" style="cursor: pointer; display: none;" class="col_id"><i class="sort_icon" id="sorticon_id"></i>Id</th> 
							<th data-bind="click:sortby.bind($data, 'tw_projects.project_name')" style="cursor: pointer; display: none;" class="col_project_name"><i class="sort_icon" id="sorticon_project_name"></i>Project Name</th> 
							<th data-bind="click:sortby.bind($data, 'tw_projects.create_date')" style="cursor: pointer; display: none;" class="col_create_date"><i class="sort_icon" id="sorticon_create_date"></i>Create date</th> 
							<th data-bind="click:sortby.bind($data, 'tw_projects.modify_date')" style="cursor: pointer; display: none;" class="col_modify_date"><i class="sort_icon" id="sorticon_modify_date"></i>Modify date</th> 
							<th>Action</th>				
						</tr>
					</thead>
					<tbody data-bind="foreach:tw_projects">
						<tr>
							<td class="center sorting_1">
								<label><input type="checkbox" class="title idcheck" name="id[]"  data-bind="attr:{value: id}" id="id[]" value="" /><span class="lbl"></span></label>
										...
										<td data-bind="text: project_name" class="col_project_name" style="display: none;"></td> 
...
<div class="control-group">
            <label class="control-label" > Project Name</label>
            <div class="controls">
              <input  class="span4"  value="" type="text"  id="pag_search_project_name" name="project_name" placeholder="Project Name" required>
			  </div>
          </div>
...
// CHECKLIST MODEL 
	function AppViewModeltw_projects(tw_projects) {
	var self = this;
		self.tw_projects = tw_projects;
	...
	// ADD NEW ITEM
   self.addtw_projects = function() {
		var fname="";
        self.tw_projects.push({ id: ko.observable(),checklist_name: ko.observable() });
		self.savetw_projects();
    };
	...
	self.deleteselecteddata = function(){
			$.ajax({
			type: "POST",
			url: "services/tw_projects_delete.php",
	...
	self.savenew = function(){
		$.ajax({
		  type: "POST",
		   url: "services/tw_projects_new.php",
	...
	// Looping up all the filters start here. 
		...
		$.ajax({
					type: "POST",
					url: "services/tw_projects_fetch.php",
					data : pag_filters,
					dataType: 'json'
					}).done(
					function( data ) 
					{
						...
						ko.mapping.fromJS(viewModel, self.tw_projects);
	...
	// SAVE THE MODEL
	self.savetw_projects = function() {
		$('#message').show();
		//alert(ko.toJSON(self.tw_projects));
		mydataa = ko.toJSON(self.tw_projects);
		$.ajax({
		  type: "POST",
		  url: "services/tw_projects_update.php",
		  data: { mydata: mydataa }
			}).done(function( msg ) {
...
<script language="javascript">
$(document).ready(function(){
	var tw_projects = new AppViewModeltw_projects(ko.mapping.fromJS([]));
	ko.applyBindings((tw_projects),$("#section_tw_projects")[0]);
	tw_projects.loaddata();
	tw_projects.update_pagination();
});
</script>
<script type="text/javascript">
$(document).ready(function() {
