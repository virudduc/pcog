...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title>Value Blank - <?php echo SITENAME; ?></title>
 ...
 <span class="hidden-phone"> Columns </span> <b class="caret"></b> </div>
	  ...
	  <button data-target="#myModal" data-toggle="modal" id="addnewrec"  class="pull-right btn btn-small btn-primary">Add</button>
							<h1>Product Categories<small><i class="icon-double-angle-right"></i> Management2</small></h1>
						</div><!--/page-header-->
						<div class="row-fluid" >
<!-- PAGE CONTENT BEGINS HERE -->
<div style="100%" id="section_Value Blank">
<div class="row-fluid" >
		...
		<div class="control-group">
			...
			<form name="maintableform" id="maintableform">
				<table id="table_report" class="table table-striped table-bordered table-hover">
					...
					<tbody data-bind="foreach:Value Blank">
						<tr>
...
// CHECKLIST MODEL 
	function AppViewModelValue Blank(Value Blank) {
	var self = this;
		self.Value Blank = Value Blank;
	...
	// ADD NEW ITEM
   self.addValue Blank = function() {
		var fname="";
        self.Value Blank.push({ id: ko.observable(),checklist_name: ko.observable() });
		self.saveValue Blank();
    };
	...
	self.deleteselecteddata = function(){
			$.ajax({
			type: "POST",
			url: "services/Value Blank_delete.php",
	...
	self.savenew = function(){
		$.ajax({
		  type: "POST",
		   url: "services/Value Blank_new.php",
	...
	// Looping up all the filters start here. 
		...
		$.ajax({
					type: "POST",
					url: "services/Value Blank_fetch.php",
					data : pag_filters,
					dataType: 'json'
					}).done(
					function( data ) 
					{
						...
						ko.mapping.fromJS(viewModel, self.Value Blank);
	...
	// SAVE THE MODEL
	self.saveValue Blank = function() {
		$('#message').show();
		//alert(ko.toJSON(self.Value Blank));
		mydataa = ko.toJSON(self.Value Blank);
		$.ajax({
		  type: "POST",
		  url: "services/Value Blank_update.php",
		  data: { mydata: mydataa }
			}).done(function( msg ) {
...
<script language="javascript">
$(document).ready(function(){
	var Value Blank = new AppViewModelValue Blank(ko.mapping.fromJS([]));
	ko.applyBindings((Value Blank),$("#section_Value Blank")[0]);
	Value Blank.loaddata();
	Value Blank.update_pagination();
});
</script>
<script type="text/javascript">
$(document).ready(function() {
