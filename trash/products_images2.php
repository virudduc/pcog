...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title>Products_images - <?php echo SITENAME; ?></title>
			...
			<div id="main-content" class="clearfix">
					<div id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li class="active">Products Images</li>
						</ul><!--.breadcrumb-->
						<div id="nav-search">
 ...
 <span class="hidden-phone"> Columns </span> <b class="caret"></b> </div>
        <ul class="dropdown-menu">
		  ...
		  <label class="checkbox">
				...
				<input type="checkbox" class="columncontrol" checked="checked" value="product_id"><span class="lbl"> Product Name </span></label>  <label class="checkbox">
				...
				<input type="checkbox" class="columncontrol" checked="checked" value="image_title"><span class="lbl"> Image Title </span></label>  <label class="checkbox">
				<input type="checkbox" class="columncontrol" checked="checked" value="image_desc"><span class="lbl"> Image Description </span></label>  <label class="checkbox">
				<input type="checkbox" class="columncontrol" checked="checked" value="image_path"><span class="lbl"> Image Path </span></label>
          </li>
        </ul>
      </span>
	  <button data-target="#myModal" data-toggle="modal" id="addnewrec"  class="pull-right btn btn-small btn-primary">Add</button>
							<h1>Products Images<small><i class="icon-double-angle-right"></i> Management</small></h1>
						</div><!--/page-header-->
						<div class="row-fluid" >
<!-- PAGE CONTENT BEGINS HERE -->
<div style="100%" id="section_products_images">
<div class="row-fluid" >
	...
	<!--  ACTION BOX ENDS HERE -->
		...
		<div class="control-group">
				...
				<div class="controls ">
					<input type="text"  id="pag_search_cms_products_images--id" class="pag_filter input-medium search-query " placeholder="Search Id">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Product Name</label>
				<div class="controls ">
					<input type="text"  id="pag_search_cms_products_images--product_id" class="pag_filter input-medium search-query " placeholder="Search Product Name">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > User Name</label>
				<div class="controls ">
					<input type="text"  id="pag_search_cms_products_images--user_id" class="pag_filter input-medium search-query " placeholder="Search User Name">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Image Title</label>
				<div class="controls ">
					<input type="text"  id="pag_search_cms_products_images--image_title" class="pag_filter input-medium search-query " placeholder="Search Image Title">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Image Description</label>
				<div class="controls ">
					<input type="text"  id="pag_search_cms_products_images--image_desc" class="pag_filter input-medium search-query " placeholder="Search Image Description">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Image Path</label>
				<div class="controls ">
					<input type="text"  id="pag_search_cms_products_images--image_path" class="pag_filter input-medium search-query " placeholder="Search Image Path">
				</div>
		</div>
								<div style="clear:both;"></div>
                                <div class="span6">
				...
				<table id="table_report" class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							...
							<th data-bind="click:sortby.bind($data, 'cms_products_images.product_id')" style="cursor: pointer; display: none;" class="col_product_id"><i class="sort_icon" id="sorticon_product_id"></i>Product Name</th> 
							<th data-bind="click:sortby.bind($data, 'cms_products_images.user_id')" style="cursor: pointer; display: none;" class="col_user_id"><i class="sort_icon" id="sorticon_user_id"></i>User Name</th> 
							<th data-bind="click:sortby.bind($data, 'cms_products_images.image_title')" style="cursor: pointer; display: none;" class="col_image_title"><i class="sort_icon" id="sorticon_image_title"></i>Image Title</th> 
							<th data-bind="click:sortby.bind($data, 'cms_products_images.image_desc')" style="cursor: pointer; display: none;" class="col_image_desc"><i class="sort_icon" id="sorticon_image_desc"></i>Image Description</th> 
							<th data-bind="click:sortby.bind($data, 'cms_products_images.image_path')" style="cursor: pointer; display: none;" class="col_image_path"><i class="sort_icon" id="sorticon_image_path"></i>Image Path</th> 
							<th>Action</th>				
						</tr>
					</thead>
					<tbody data-bind="foreach:products_images">
						<tr>
							<td class="center sorting_1">
								<label><input type="checkbox" class="title idcheck" name="id[]"  data-bind="attr:{value: id}" id="id[]" value="" /><span class="lbl"></span></label>
							</td>
										<td data-bind="text: product_id" class="col_product_id" style="display: none;"></td> 
										<td data-bind="text: user_id" class="col_user_id" style="display: none;"></td> 
										<td data-bind="text: image_title" class="col_image_title" style="display: none;"></td> 
										<td data-bind="text: image_desc" class="col_image_desc" style="display: none;"></td> 
							<td>
...
<div class="control-group">
            <label class="control-label" >Product Name</label>
            <div class="controls">
          <select  id="pag_search_product_id" name="product_id" placeholder="Product Name" >
			<?php
				$cms_products_query ="select * from cms_products";
				$cms_products_rs = mysql_query($cms_products_query) or die(mysql_error()."</br> Failed Query".$project_query );
				//Loop Placed here. 
				while($cms_products_row = mysql_fetch_array($cms_products_rs))
				{
				?> 
				<option value="<?php echo $cms_products_row['id'] ;?>"><?php echo $cms_products_row['product_name'] ;?></option>
				<?php 
				}
			?>
         </select>
              ...
              <input type="hidden" value="$_SESSION['userinfo']['user_id']"  id="pag_search_user_id" name="user_id" placeholder="User Name" required>
<div class="control-group">
            <label class="control-label" > Image Title</label>
            <div class="controls">
              <input  class="span4"  value="" type="text"  id="pag_search_image_title" name="image_title" placeholder="Image Title" required>
			  </div>
          </div>
<div class="control-group">
            <label class="control-label" > Image Description</label>
            <div class="controls">
       <textarea class="span4" id="pag_search_image_desc" name="image_desc" placeholder="Image Description" ></textarea>
	   </div>
          </div>
        </form>
      </div>
...
// CHECKLIST MODEL 
	function AppViewModelproducts_images(products_images) {
	var self = this;
	...
	// ADD NEW ITEM
   self.addproducts_images = function() {
		var fname="";
        self.products_images.push({ id: ko.observable(),checklist_name: ko.observable() });
		self.saveproducts_images();
    };
	...
	self.deleteselecteddata = function(){
			$.ajax({
			type: "POST",
			url: "services/products_images_delete.php",
	...
	self.savenew = function(){
		$.ajax({
		  type: "POST",
		   url: "services/products_images_new.php",
	...
	// Looping up all the filters start here. 
		...
		$.ajax({
					type: "POST",
					url: "services/products_images_fetch.php",
					data : pag_filters,
					dataType: 'json'
					}).done(
					function( data ) 
					{
						...
						ko.mapping.fromJS(viewModel, self.products_images);
	...
	// SAVE THE MODEL
	self.saveproducts_images = function() {
		$('#message').show();
		//alert(ko.toJSON(self.products_images));
		mydataa = ko.toJSON(self.products_images);
		$.ajax({
		  type: "POST",
		  url: "services/products_images_update.php",
		  data: { mydata: mydataa }
			}).done(function( msg ) {
	...
	self.removeItem = function() {
        self.products_images.remove(this);
...
<script language="javascript">
$(document).ready(function(){
	var products_images = new AppViewModelproducts_images(ko.mapping.fromJS([]));
	ko.applyBindings((products_images),$("#section_products_images")[0]);
	products_images.loaddata();
	products_images.update_pagination();
});
</script>
<script type="text/javascript">
$(document).ready(function() {
