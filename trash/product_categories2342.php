				...
				<table id="table_report" class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							...
							<th data-bind="click:sortby.bind($data, 'cms_product_categories.lft')" style="cursor: pointer; display: none;" class="col_lft"><i class="sort_icon" id="sorticon_lft"></i></th> 
							<th data-bind="click:sortby.bind($data, 'cms_product_categories.rgt')" style="cursor: pointer; display: none;" class="col_rgt"><i class="sort_icon" id="sorticon_rgt"></i></th> 
							<th>Action</th>				
						</tr>
					</thead>
						...
						<tr>
							<td class="center sorting_1">
								<label><input type="checkbox" class="title idcheck" name="id[]"  data-bind="attr:{value: id}" id="id[]" value="" /><span class="lbl"></span></label>
										...
										<td data-bind="text: lft" class="col_lft" style="display: none;"></td> 
										<td data-bind="text: rgt" class="col_rgt" style="display: none;"></td> 
							<td>
...
<div class="control-group">
            <label class="control-label" >Parent Category</label>
            <div class="controls">
          <select  id="pag_search_pid" name="pid" placeholder="Parent Category" >
			<?php
...
// CHECKLIST MODEL 
   ...
   self.addproduct_categories = function() {
	...
	self.deleteselecteddata = function(){
			$.ajax({
			type: "POST",
			url: "services/product_categories234_delete.php",
	...
	self.savenew = function(){
		$.ajax({
		  type: "POST",
		   url: "services/product_categories234_new.php",
	...
	// Looping up all the filters start here. 
		...
		$.ajax({
					type: "POST",
					url: "services/product_categories234_fetch.php",
					data : pag_filters,
					dataType: 'json'
					}).done(
					function( data ) 
	...
	self.saveproduct_categories = function() {
		...
		$.ajax({
		  type: "POST",
		  url: "services/product_categories234_update.php",
		  data: { mydata: mydataa }
			}).done(function( msg ) {
