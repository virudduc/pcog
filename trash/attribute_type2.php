...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title>Attribute_type - <?php echo SITENAME; ?></title>
			...
			<div id="main-content" class="clearfix">
					<div id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li class="active">Attribute Type</li>
						</ul><!--.breadcrumb-->
						<div id="nav-search">
 ...
 <span class="hidden-phone"> Columns </span> <b class="caret"></b> </div>
        <ul class="dropdown-menu">
		  ...
		  <label class="checkbox">
				...
				<input type="checkbox" class="columncontrol" checked="checked" value="attribute_type"><span class="lbl"> Attribute type </span></label>
          </li>
        </ul>
      </span>
	  <button data-target="#myModal" data-toggle="modal" id="addnewrec"  class="pull-right btn btn-small btn-primary">Add</button>
							<h1>Attribute Type<small><i class="icon-double-angle-right"></i> Attribute Management</small></h1>
						</div><!--/page-header-->
						<div class="row-fluid" >
<!-- PAGE CONTENT BEGINS HERE -->
<div style="100%" id="section_attribute_type">
<div class="row-fluid" >
	...
	<!--  ACTION BOX ENDS HERE -->
		...
		<div class="control-group">
				...
				<div class="controls ">
					<input type="text"  id="pag_search_attribute_type--id" class="pag_filter input-medium search-query " placeholder="Search Id">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Attribute type</label>
				<div class="controls ">
					<input type="text"  id="pag_search_attribute_type--attribute_type" class="pag_filter input-medium search-query " placeholder="Search Attribute type">
				</div>
		</div>
								<div style="clear:both;"></div>
                                <div class="span6">
				...
				<table id="table_report" class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							...
							<th data-bind="click:sortby.bind($data, 'attribute_type.attribute_type')" style="cursor: pointer; display: none;" class="col_attribute_type"><i class="sort_icon" id="sorticon_attribute_type"></i>Attribute type</th> 
							<th>Action</th>				
						</tr>
					</thead>
					<tbody data-bind="foreach:attribute_type">
						<tr>
...
<div class="control-group">
            <label class="control-label" > Attribute type</label>
            <div class="controls">
              <input  class="span4"  value="" type="text"  id="pag_search_attribute_type" name="attribute_type" placeholder="Attribute type" required>
			  </div>
          </div>
        </form>
      </div>
...
// CHECKLIST MODEL 
	function AppViewModelattribute_type(attribute_type) {
	var self = this;
	...
	// ADD NEW ITEM
   self.addattribute_type = function() {
		var fname="";
        self.attribute_type.push({ id: ko.observable(),checklist_name: ko.observable() });
		self.saveattribute_type();
    };
	...
	self.deleteselecteddata = function(){
			$.ajax({
			type: "POST",
			url: "services/attribute_type_delete.php",
	...
	self.savenew = function(){
		$.ajax({
		  type: "POST",
		   url: "services/attribute_type_new.php",
	...
	// Looping up all the filters start here. 
		...
		$.ajax({
					type: "POST",
					url: "services/attribute_type_fetch.php",
					data : pag_filters,
					dataType: 'json'
					}).done(
					function( data ) 
					{
						...
						ko.mapping.fromJS(viewModel, self.attribute_type);
	...
	// SAVE THE MODEL
	self.saveattribute_type = function() {
		$('#message').show();
		//alert(ko.toJSON(self.attribute_type));
		mydataa = ko.toJSON(self.attribute_type);
		$.ajax({
		  type: "POST",
		  url: "services/attribute_type_update.php",
		  data: { mydata: mydataa }
			}).done(function( msg ) {
	...
	self.removeItem = function() {
        self.attribute_type.remove(this);
...
<script language="javascript">
$(document).ready(function(){
	var attribute_type = new AppViewModelattribute_type(ko.mapping.fromJS([]));
	ko.applyBindings((attribute_type),$("#section_attribute_type")[0]);
	attribute_type.loaddata();
	attribute_type.update_pagination();
});
</script>
<script type="text/javascript">
$(document).ready(function() {
