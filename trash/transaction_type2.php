...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title>Transaction_type - <?php echo SITENAME; ?></title>
			...
			<div id="main-content" class="clearfix">
					<div id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li class="active">Credit Transaction Type</li>
						</ul><!--.breadcrumb-->
						<div id="nav-search">
 ...
 <span class="hidden-phone"> Columns </span> <b class="caret"></b> </div>
        <ul class="dropdown-menu">
		  ...
		  <label class="checkbox">
				...
				<input type="checkbox" class="columncontrol" checked="checked" value="transaction_type"><span class="lbl"> Transaction Type </span></label>  <label class="checkbox">
				<input type="checkbox" class="columncontrol" checked="checked" value="description"><span class="lbl"> Description </span></label>
          </li>
        </ul>
      </span>
	  <button data-target="#myModal" data-toggle="modal" id="addnewrec"  class="pull-right btn btn-small btn-primary">Add</button>
							<h1>Credit Transaction Type<small><i class="icon-double-angle-right"></i> Management</small></h1>
						</div><!--/page-header-->
						<div class="row-fluid" >
<!-- PAGE CONTENT BEGINS HERE -->
<div style="100%" id="section_transaction_type">
<div class="row-fluid" >
	...
	<!--  ACTION BOX ENDS HERE -->
		...
		<div class="control-group">
				...
				<div class="controls ">
					<input type="text"  id="pag_search_sub_credit_transaction_type--id" class="pag_filter input-medium search-query " placeholder="Search Id">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Transaction Type</label>
				<div class="controls ">
					<input type="text"  id="pag_search_sub_credit_transaction_type--transaction_type" class="pag_filter input-medium search-query " placeholder="Search Transaction Type">
				</div>
		</div>
		<div class="control-group">
				<label class="control-label " > Description</label>
				<div class="controls ">
					<input type="text"  id="pag_search_sub_credit_transaction_type--description" class="pag_filter input-medium search-query " placeholder="Search Description">
				</div>
		</div>
								<div style="clear:both;"></div>
                                <div class="span6">
				...
				<table id="table_report" class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							...
							<th data-bind="click:sortby.bind($data, 'sub_credit_transaction_type.transaction_type')" style="cursor: pointer; display: none;" class="col_transaction_type"><i class="sort_icon" id="sorticon_transaction_type"></i>Transaction Type</th> 
							<th data-bind="click:sortby.bind($data, 'sub_credit_transaction_type.description')" style="cursor: pointer; display: none;" class="col_description"><i class="sort_icon" id="sorticon_description"></i>Description</th> 
							<th>Action</th>				
						</tr>
					</thead>
					<tbody data-bind="foreach:transaction_type">
						<tr>
							<td class="center sorting_1">
								<label><input type="checkbox" class="title idcheck" name="id[]"  data-bind="attr:{value: id}" id="id[]" value="" /><span class="lbl"></span></label>
							</td>
										<td data-bind="text: transaction_type" class="col_transaction_type" style="display: none;"></td> 
										<td data-bind="text: description" class="col_description" style="display: none;"></td> 
							<td>
...
<div class="control-group">
            <label class="control-label" > Transaction Type</label>
            <div class="controls">
              <input  class="span4"  value="" type="text"  id="pag_search_transaction_type" name="transaction_type" placeholder="Transaction Type" required>
			  </div>
          </div>
<div class="control-group">
            <label class="control-label" > Description</label>
            <div class="controls">
              <input  class="span4"  value="" type="text"  id="pag_search_description" name="description" placeholder="Description" required>
			  </div>
          </div>
        </form>
      </div>
...
// CHECKLIST MODEL 
	function AppViewModeltransaction_type(transaction_type) {
	var self = this;
	...
	// ADD NEW ITEM
   self.addtransaction_type = function() {
		var fname="";
        self.transaction_type.push({ id: ko.observable(),checklist_name: ko.observable() });
		self.savetransaction_type();
    };
	...
	self.deleteselecteddata = function(){
			$.ajax({
			type: "POST",
			url: "services/transaction_type_delete.php",
	...
	self.savenew = function(){
		$.ajax({
		  type: "POST",
		   url: "services/transaction_type_new.php",
	...
	// Looping up all the filters start here. 
		...
		$.ajax({
					type: "POST",
					url: "services/transaction_type_fetch.php",
					data : pag_filters,
					dataType: 'json'
					}).done(
					function( data ) 
					{
						...
						ko.mapping.fromJS(viewModel, self.transaction_type);
	...
	// SAVE THE MODEL
	self.savetransaction_type = function() {
		$('#message').show();
		//alert(ko.toJSON(self.transaction_type));
		mydataa = ko.toJSON(self.transaction_type);
		$.ajax({
		  type: "POST",
		  url: "services/transaction_type_update.php",
		  data: { mydata: mydataa }
			}).done(function( msg ) {
	...
	self.removeItem = function() {
        self.transaction_type.remove(this);
...
<script language="javascript">
$(document).ready(function(){
	var transaction_type = new AppViewModeltransaction_type(ko.mapping.fromJS([]));
	ko.applyBindings((transaction_type),$("#section_transaction_type")[0]);
	transaction_type.loaddata();
	transaction_type.update_pagination();
});
</script>
<script type="text/javascript">
$(document).ready(function() {
