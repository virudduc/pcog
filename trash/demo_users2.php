...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title>Demo_users - <?php echo SITENAME; ?></title>
 ...
 <span class="hidden-phone"> Columns </span> <b class="caret"></b> </div>
        <ul class="dropdown-menu">
		  ...
		  <label class="checkbox">
				...
				<input type="checkbox" class="columncontrol" checked="checked" value="phone"><span class="lbl">  </span></label>  <label class="checkbox">
				...
				<input type="checkbox" class="columncontrol" checked="checked" value="code"><span class="lbl">  </span></label>  <label class="checkbox">
	  ...
	  <button data-target="#myModal" data-toggle="modal" id="addnewrec"  class="pull-right btn btn-small btn-primary">Add</button>
							<h1>Demo_users<small><i class="icon-double-angle-right"></i> Demo_users</small></h1>
						</div><!--/page-header-->
						<div class="row-fluid" >
<!-- PAGE CONTENT BEGINS HERE -->
<div style="100%" id="section_demo_users">
<div class="row-fluid" >
	...
	<!--  ACTION BOX ENDS HERE -->
		...
		<div class="control-group">
				...
				<div class="controls ">
					<input type="text"  id="pag_search_vt_users--phone" class="pag_filter input-medium search-query " placeholder="Search ">
				</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
				<label class="control-label " > </label>
				<div class="controls ">
					<input type="text"  id="pag_search_vt_users--code" class="pag_filter input-medium search-query " placeholder="Search ">
				</div>
		</div>
		<div class="control-group">
				...
				<table id="table_report" class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							...
							<th data-bind="click:sortby.bind($data, 'vt_users.mobile')" style="cursor: pointer; display: none;" class="col_mobile"><i class="sort_icon" id="sorticon_mobile"></i>Mobile No.</th> 
							<th data-bind="click:sortby.bind($data, 'vt_users.email')" style="cursor: pointer; display: none;" class="col_email"><i class="sort_icon" id="sorticon_email"></i>Email</th> 
							<th data-bind="click:sortby.bind($data, 'vt_users.profile_image')" style="cursor: pointer; display: none;" class="col_profile_image"><i class="sort_icon" id="sorticon_profile_image"></i>Profile Image</th> 
							<th data-bind="click:sortby.bind($data, 'vt_users.gender')" style="cursor: pointer; display: none;" class="col_gender"><i class="sort_icon" id="sorticon_gender"></i>Gender</th> 
					...
					<tbody data-bind="foreach:demo_users">
						<tr>
							<td class="center sorting_1">
								<label><input type="checkbox" class="title idcheck" name="id[]"  data-bind="attr:{value: id}" id="id[]" value="" /><span class="lbl"></span></label>
										...
										<td data-bind="text: mobile" class="col_mobile" style="display: none;"></td> 
										<td data-bind="text: email" class="col_email" style="display: none;"></td> 
										<td data-bind="text: gender" class="col_gender" style="display: none;"></td> 
...
<div class="control-group">
            ...
            <div class="controls">
              ...
              <input type="hidden" value=""  id="pag_search_phone" name="phone" placeholder="" required>
<div class="control-group">
            <label class="control-label" > Email</label>
            <div class="controls">
...
<div class="control-group">
         ...
         <input type="text"  value=""  id="pag_search_birth_date" class="bootdatepickersingle"  name="birth_date" placeholder="Birth Date" required>
              ...
              <input type="hidden" value=""  id="pag_search_code" name="code" placeholder="" required>
<div class="control-group">
            <label class="control-label" >Gender</label>
            <div class="controls">
...
// CHECKLIST MODEL 
	function AppViewModeldemo_users(demo_users) {
	var self = this;
	...
	// ADD NEW ITEM
   self.adddemo_users = function() {
		var fname="";
        self.demo_users.push({ id: ko.observable(),checklist_name: ko.observable() });
		self.savedemo_users();
    };
	...
	self.deleteselecteddata = function(){
			$.ajax({
			type: "POST",
			url: "services/demo_users_delete.php",
	...
	self.savenew = function(){
		$.ajax({
		  type: "POST",
		   url: "services/demo_users_new.php",
	...
	// Looping up all the filters start here. 
		...
		$.ajax({
					type: "POST",
					url: "services/demo_users_fetch.php",
					data : pag_filters,
					dataType: 'json'
					}).done(
					function( data ) 
					{
						...
						ko.mapping.fromJS(viewModel, self.demo_users);
	...
	// SAVE THE MODEL
	self.savedemo_users = function() {
		$('#message').show();
		//alert(ko.toJSON(self.demo_users));
		mydataa = ko.toJSON(self.demo_users);
		$.ajax({
		  type: "POST",
		  url: "services/demo_users_update.php",
		  data: { mydata: mydataa }
			}).done(function( msg ) {
...
<script language="javascript">
$(document).ready(function(){
	var demo_users = new AppViewModeldemo_users(ko.mapping.fromJS([]));
	ko.applyBindings((demo_users),$("#section_demo_users")[0]);
	demo_users.loaddata();
	demo_users.update_pagination();
});
</script>
<script type="text/javascript">
$(document).ready(function() {
